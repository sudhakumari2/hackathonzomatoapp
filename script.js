...
function order_food() {
    ...
    var restraurant = readlineSync.question("These are  the avalibal restraurant you can select any one:--")
    your_resto = {
        ...
        if (restraurant === myfood) {
            ...
            for (var i = 0; i < your_resto[myfood].length; i++) {
                ...
                my_favfood = readlineSync.question("Choose any intem according to the menu list:--");
                  for (key in your_resto[myfood][i]) {
                    if (my_favfood === key) {
                        console.log("your order is coming within few minutes", " your total payment is", your_resto[myfood][i][key]);
                    }
                }
